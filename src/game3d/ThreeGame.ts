import type { RunMode, SaveData, StageDefinition } from '../types';
import { SurvivorGame3D } from './SurvivorGame3D';
import type { Game3DCallbacks } from './types';

let activeGame: SurvivorGame3D | undefined;

export function mountThreeGame(
  parent: HTMLElement,
  stage: StageDefinition,
  save: SaveData,
  callbacks: Game3DCallbacks,
  mode: RunMode = 'campaign'
): SurvivorGame3D | undefined {
  destroyThreeGame();
  try {
    activeGame = new SurvivorGame3D({ parent, stage, save, callbacks, mode });
    activeGame.start();
    return activeGame;
  } catch (error) {
