...
export interface CombatAimVector { x: number; y: number }
export interface CombatFlowState { meter: number; overdrive: boolean; overdriveRemaining: number; streak: number }
...
let combatFlowState: CombatFlowState = { meter: 0, overdrive: false, overdriveRemaining: 0, streak: 0 };
...
export function getCombatAutoAim(maxAgeMs = 350): CombatAimVector | undefined {
  if (!autoAim || performance.now() - autoAimUpdatedAt > maxAgeMs) return undefined;
  return { ...autoAim };
}

export function clearCombatAutoAim(): void { autoAim = undefined; autoAimUpdatedAt = 0; }
...
export function setCombatFlowState(next: CombatFlowState): void {
  ...
  const changed = Math.abs(normalized.meter - combatFlowState.meter) >= 0.5 ||
  ...
  if (changed && typeof window !== 'undefined') {
    window.dispatchEvent(new CustomEvent<CombatFlowState>('tiny-survivor-combat-flow', { detail: { ...combatFlowState } }));
  }
}

export function getCombatFlowState(): CombatFlowState { return { ...combatFlowState }; }

export function announceCombatWave(announcement: CombatWaveAnnouncement): void {
  if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent<CombatWaveAnnouncement>('tiny-survivor-wave', { detail: announcement }));
}
