...
export class WeaponSystem {
  ...
  update(delta: number): void {
    ...
    this.autoTarget = this.findAutoTarget(590);
    if (this.autoTarget) {
    ...
    if (boltCooldown <= 0) {
      if (primaryFiring && this.autoTarget && this.triggerMagicBolt(this.autoTarget)) {
        ...
        this.cooldowns.set('magic-bolt', Math.max(0.08, base * levelRate * overdriveRate));
      } else {
    ...
    if (this.hasWeapon('orbiting-blades')) {
      ...
      if (bladesCooldown <= 0) {
        ...
        const overdriveBoost = this.overdriveRemaining > 0 ? 1.16 : 1;
        const damage =
    ...
    if (this.hasWeapon('chain-lightning')) {
      ...
      if (this.chainFallbackTimer >= fallbackThreshold) {
        ...
        const nearby = this.hooks
          .findTargetsInRadius(player.x, player.y, 340)
          .sort((a, b) =>
  ...
  private updateFlow(delta: number): void {
    ...
    if (this.flowBroadcastTimer <= 0) {
      this.broadcastFlow();
      this.flowBroadcastTimer = 0.1;
    }
  }

  private broadcastFlow(): void {
    setCombatFlowState({
      meter: this.flowMeter,
  ...
  private findAutoTarget(maxDist: number): TargetPoint | undefined {
    ...
    if (locked) {
      ...
      if (!nearest || lockedDistance <= 155 || nearestDistance >= lockedDistance * 0.58) {
        return locked;
      }
    }
  ...
  private triggerMagicBolt(target: TargetPoint): boolean {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      this.hooks.fireProjectile({
        ...
        damage,
        speed: empowered ? 465 : overdrive ? 450 : 420,
        radius: empowered ? 10 : 7,
        pierce,
        color: empowered ? 0xffd166 : WEAPON_BALANCE['magic-bolt'].color,
  ...
  private triggerChainLightningAt(originX: number, originY: number, initialTargetId?: string): void {
    ...
    const damage =
      ...
      this.hooks.getAutoWeaponDamageMultiplier() *
      (1 + (level - 1) * 0.25) *
      (this.overdriveRemaining > 0 ? 1.18 : 1);
    ...
    const nearby = this.hooks
      ...
      .filter((target) => target.id !== initialPoint.id)
      .sort((a, b) =>
        Math.hypot(a.x - originX, a.y - originY) -
