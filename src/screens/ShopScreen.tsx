import { Check, Coins, Gem, Gift, Play, Shield, Sparkles, Swords, Zap } from 'lucide-react';
...
export function ShopScreen({ save, onBack, onFreeChest, onBuyHero, onBuyEquipment }: ShopScreenProps) {
  ...
  // Filter equipment for the active slot
    ...
    <main className="meta-screen shop-landscape-screen">
      ...
      <div className="shop-landscape-container">
        ...
        <nav className="shop-tabs-bar" aria-label="Shop categories">
          <button
            type="button"
            className={`shop-tab-btn ${activeTab === 'daily' ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className={`shop-tab-btn ${activeTab === 'heroes' ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className={`shop-tab-btn ${activeTab === 'armor' ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className={`shop-tab-btn ${activeTab === 'relic' ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className={`shop-tab-btn ${activeTab === 'pet' ? 'is-active' : ''}`}
          ...
          <button
            type="button"
            className={`shop-tab-btn ${activeTab === 'charm' ? 'is-active' : ''}`}
        ...
        <div className="shop-tab-content">
            ...
            <div className="shop-daily-panel">
              <section className="free-chest-card">
                ...
                <div className="free-chest-card__copy">
                  ...
                  <p>
                    <Coins size={15} /> Watch one rewarded message each day to claim +120 bonus gold.
                  </p>
                </div>
                <PrimaryButton
                  variant="gold"
            ...
            <div className="shop-grid shop-grid--heroes">
              {ALL_HERO_IDS.map((heroId) => {
                ...
                return (
                  <div
                    key={heroId}
                    className={`shop-card shop-card--hero shop-card--${def.tone} ${
                      owned ? 'is-owned' : ''
                    }`}
                  >
                    ...
                    <div className="shop-card__body">
                      <div className="shop-card__title-row">
                        ...
                        <span className={`shop-card__gender-badge shop-card__gender--${def.gender}`}>
                          {def.gender.toUpperCase()}
                        </span>
                      </div>
          ...
          {/* EQUIPMENT SLOTS (ARMOR, RELIC, PET, CHARM) */}
          {(activeTab === 'armor' ||
            activeTab === 'relic' ||
            activeTab === 'pet' ||
            activeTab === 'charm') && (
            <div className="shop-grid shop-grid--equipment">
              {currentSlotEquipment.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`shop-card shop-card--equipment item-rarity-border--${item.rarity} ${
                      owned ? 'is-owned' : ''
                    }`}
                  >
                    ...
                    <div className="shop-card__body">
                      ...
                      {item.bossDropFrom && (
                        <span className="shop-card__drop-source">
                          World Boss First Clear Drop
                        </span>
                      )}
                    </div>
