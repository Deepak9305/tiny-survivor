    ...
    <main className="meta-screen missions-landscape-screen">
      ...
      <div className="missions-body-container">
        <section className="missions-top-banner">
          ...
          <div className="missions-top-banner__ornament"><span className="missions-top-banner__diamond" /></div>
          <div className="missions-top-banner__content">
            <div className="missions-top-banner__left">
              <div className="missions-top-banner__icon-circle">
                <svg viewBox="0 0 24 24" width="26" height="26" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        <section className="missions-active-section">
          <div className="missions-section-bar">
            ...
            <span className="missions-section-sub">3 daily objectives</span>
          </div>
          ...
          <div className="missions-cards-list">
            {missions.map((mission) => {
              ...
              const percent = mission.target > 0 ? Math.min(100, Math.round((mission.progress / mission.target) * 100)) : 0;
              const isDone = mission.progress >= mission.target;

              return (
                <article
                  key={mission.id}
                  className={`mission-card-landscape ${isDone ? 'is-complete' : ''} ${mission.claimed ? 'is-claimed' : ''}`}
                ...
                >
                  ...
                  <div className="mission-card-landscape__content">
                    <div className="mission-card-landscape__icon-circle"><Icon size={24} /></div>
                    <div className="mission-card-landscape__body">
                      <div className="mission-card-landscape__title-row">
                        ...
                        <span className="mission-card-landscape__numbers">{currentProgress.toLocaleString()} / {mission.target.toLocaleString()}</span>
                      </div>
                      <p className="mission-card-landscape__subtitle">{meta.subtitle}</p>
                      <div className="mission-card-landscape__track" aria-label={`${percent}% complete`}>
                        <div className="mission-card-landscape__fill" style={{ width: `${percent}%` }} />
                      </div>
                    </div>
                    <div className="mission-card-landscape__action">
                      <button
                        type="button"
                        className={`mission-reward-gold-btn ${mission.claimed ? 'is-claimed' : isDone ? 'is-ready' : 'is-pending'}`}
                        disabled={!isDone || mission.claimed}
                        onClick={() => onClaim(mission.id)}
                        aria-label={mission.claimed ? `${mission.title}, claimed` : `${mission.title}, reward ${mission.reward} coins`}
                      >
                        {mission.claimed ? <><Check size={16} /><span>CLAIMED</span></> : <><span className="mission-reward-gold-btn__coin-circle"><Coins size={14} /></span><span className="mission-reward-gold-btn__val">{mission.reward}</span></>}
                      </button>
                    </div>
                  </div>
                </article>
