...
export function SettingsScreen({ save, onBack, onUpdate, onReset }: SettingsScreenProps) {
  ...
  return <main className="meta-screen"><ScreenHeader title="SETTINGS" onBack={onBack} />
    ...
    <section className="settings-links"><button><span>Language</span><span>English <ChevronRight size={15} /></span></button><button><span>Privacy policy</span><ChevronRight size={15} /></button><button><span>Terms of service</span><ChevronRight size={15} /></button></section>
    <PrimaryButton variant="danger" wide onClick={() => { if (window.confirm('Reset all Tiny Survivor progress?')) onReset(); }}><RotateCcw size={16} /> RESET PROGRESS</PrimaryButton><p className="version-label">TINY SURVIVOR · v0.1.0 · Offline-first</p>
  </main>;
}

interface SettingRowProps { label: string; icon: typeof Volume2; value: boolean; onToggle: () => void }
function SettingRow({ label, icon: Icon, value, onToggle }: SettingRowProps) { return <div className="setting-row"><span><Icon size={17} /> {label}</span><button className={`toggle ${value ? 'is-on' : ''}`} onClick={onToggle} aria-label={`Toggle ${label}`}><i /></button></div>; }
